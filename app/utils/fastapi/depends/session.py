...
async def get_session(
    ...
    if basic_auth:
        ...
        user = await users_db.get_by_credentials(
            db_conn, basic_auth.username, hashed_password
        )
        if user:
            session = Session()
    ...
    return Session(session_type=COOKIE_SESSION)


async def get_from_redis(
    session_type: SessionType, key: str, redis_conn: redis.Connection
) -> Session | None:
    data = await redis.get(redis_conn, cache_key(key))
    if data is None:
        return None
    session = Session(session_type=session_type, key=key, data=data)
    return session


async def save_to_redis(session: Session, redis_conn: redis.Connection):
    await redis.setex(redis_conn, cache_key(session.key), session.max_age, session.data)


async def remove_from_redis(session: Session, redis_conn: redis.Connection):
    await redis.del_(redis_conn, cache_key(session.key))


async def get_session_user(session: Session, db_conn: db.Session) -> Session:
    user = None
