...
from app.schemas.base import ValidationError, ErrorResponse, UpdateErrorResponse
...
async def error_409(errors: list[Any]):
    return JSONResponse(
        status_code=409, content=UpdateErrorResponse(errors=errors).model_dump_json()
    )
...
async def error_500(
    detail: Optional[str] = None, debug: Optional[str] = None
) -> JSONResponse:
    return JSONResponse(
        status_code=500,
        content=ErrorResponse(
            error=detail or TRANSLATIONS_BY_STATUS.get(500), debug=debug
        ).model_dump(),
    )


async def error_400_with_detail(
    detail: Optional[str] = None, debug: Optional[str] = None
) -> JSONResponse:
    return JSONResponse(
        status_code=400,
        content=ErrorResponse(
            error=detail or TRANSLATIONS_BY_STATUS.get(400), debug=debug
        ).model_dump(),
    )
...
def register_exception_handler(app: FastAPI):
    ...
    async def update_fobidden_exception_handler(
        request: Request, exc: UpdateExpiredException
    ) -> JSONResponse:
