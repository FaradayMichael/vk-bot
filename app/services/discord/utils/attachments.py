...
async def refresh_attachments_urls(bot: Bot, urls: list[str]) -> list[str]:
    if not urls:
        return []
    ...
    if response and "refreshed_urls" in response:
        return [i["refreshed"] for i in response["refreshed_urls"]]
    logger.error(f"Failed to refresh attachments urls: {response}")
    return []


def get_video_attachment_urls_from_message(message: Message) -> list[str]:
    video_urls = get_discord_urls_from_text(message.content, VIDEO_EXT)
    if message.attachments:
        for attachment in message.attachments:
            if (
                AttachmentType.by_content_type(attachment.content_type)
...
def get_image_attachment_urls_from_message(message: Message) -> list[str]:
...
def get_yt_urls_from_message(message: Message) -> list[str]:
...
def get_discord_urls_from_text(
    text: str, patterns_set: Sequence = IMG_EXT
) -> list[str]:
    if not text:
        return []
    ...
    words = text.split()
    for word in words:
        if word.startswith(DISCORD_MEDIA_PREFIX) or word.startswith(
            DISCORD_ATTACHMENT_PREFIX
        ):
