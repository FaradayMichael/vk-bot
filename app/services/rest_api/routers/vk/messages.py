...
from app.schemas.vk import Message, SendMessage
...
async def api_send_message_vk(
    data: SendMessageInput, vk_client: VkClient = Depends(get_vk_client)
    ...
    if data.message.attachments:
        for attachment in data.message.attachments:
            async with TempBase64File(attachment.file) as tmp:
                if a := await vk_bl.file_to_vk_attachment(
                    vk_client, data.peer_id, tmp.filepath, attachment.type
                ):
    ...
    await vk_client.messages.send(data.peer_id, message)

    return SendMessageResponse(data=SendMessage(peer_id=data.peer_id, message=message))
...
async def api_get_history(
    ...
    tasks = await tasks_db.get_list(
        session=conn, from_dt=from_dt, to_dt=to_dt, funcs_in=["on_new_message"]
    )
    ...
    return MessagesHistoryResponse(total=len(result), items=result)
