...
from app.business_logic.yt import (
...
from app.services.vk_bot.models.vk import WallItemFilter
...
async def file_to_vk_attachment(
    client: VkClient, peer_id: int, file_path: str, t: AttachmentType
) -> str | None:
    match t:
        case AttachmentType.PHOTO:
            tmp = await client.upload.photos_message(peer_id, [file_path])
            result = tmp[0] if tmp else None
        case AttachmentType.DOC:
            result = await client.upload.doc_message(peer_id, file_path)
        case _ as arg:
            logger.info(arg)
            result = None
    return result
...
async def post_in_group_wall(
    ...
    post_model = WallPost(message_text=message_text, attachments=",".join(attachments))
    match mode:
        case GroupPostMode.COMPILE_9:
            ...
            available_posts = [
                p
                for p in posts
                if len(p.attachments) + len(attachments) <= 9
                and p.post_source["type"] == "api"
            ]
            if not available_posts:
                ...
                logger.info(
                    f"Edit {post.id=} for {len(attachments)} new attachments. {post_model}"
                )

                post_attachments = [
                    a.photo.attachment_str for a in post.attachments if a.photo
                ] + attachments
                ...
                await client.wall.edit(
                    ...
                    post_time=_randomize_time(
                        ...
                        + datetime.timedelta(
                            days=2 if len(post_attachments) >= 9 else 14
                        )
                    ),
                )
                if len(post_attachments) >= 9:
                    ...
                    if notify:
                        await client.messages.send(
                            peer_id=client.user_id, message=Message(text=msg)
                        )
