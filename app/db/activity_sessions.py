...
async def create(
    session: AsyncSession, model: ActivitySessionCreate
...
async def update(
    ...
    **update_data,
    ...
    stmt = (
        update_(DiscordActivitySession)
        .values(**data)
        .where(pk == DiscordActivitySession.id)
        .returning(DiscordActivitySession)
    )
...
async def delete(
    ...
    stmt = (
        delete_(DiscordActivitySession)
        .where(pk == DiscordActivitySession.id)
        .returning(DiscordActivitySession)
    )
    result = await session.execute(stmt)
...
async def get_first_unfinished(
    session: AsyncSession, user_id: str | int, activity_name: str
) -> DiscordActivitySession | None:
    user_id = str(user_id)
    stmt = (
        select(DiscordActivitySession)
        .where(
            and_(
                user_id == DiscordActivitySession.user_id,
                activity_name == DiscordActivitySession.activity_name,
            )
        )
        .order_by(DiscordActivitySession.started_at.desc())
        .limit(1)
    )
    result = await session.execute(stmt)
    return result.scalars().first()
...
async def get_list(
    ...
    if to_dt:
        where_stmts.append(
            or_(
                DiscordActivitySession.finished_at is None,
                DiscordActivitySession.started_at <= to_dt,
            )
        )
...
async def get_users_data(
    ...
    ).group_by(DiscordActivitySession.user_id, DiscordActivitySession.user_name)
    result = await session.execute(stmt)
