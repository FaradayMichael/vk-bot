...
async def create(
    session: AsyncSession, model: RegisterModel, hashed_password: str
) -> User:
    obj = User(password=hashed_password, **model.model_dump())
    session.add(obj)
    await session.commit()
    return obj


async def get(session: AsyncSession, pk: int) -> User | None:
