...
async def create(session: db.Session, model: TriggerAnswerCreate) -> TriggerAnswer:
    obj = TriggerAnswer(**model.model_dump())
    session.add(obj)
    await session.commit()
    return obj


async def delete(session: db.Session, obj: TriggerAnswer) -> None:
    await session.delete(obj)
    await session.commit()


async def get(session: db.Session, pk: int) -> TriggerAnswer | None:
...
async def get_triggers_group(session: db.Session, q: str = "") -> list[TriggerGroup]:
    stmt = (
        select(
    ...
    result = await session.execute(stmt)
    return [TriggerGroup.model_validate(i) for i in result.mappings().all()]


async def get_for_like(session: db.Session, q: str = "") -> list[TriggerGroup]:
    stmt = (
        select(
            TriggerAnswer.trigger,
            func.json_agg(
