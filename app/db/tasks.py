...
from app.schemas.vk_tasks import VkTask as VkTaskSchema


async def create(session: AsyncSession, model: VkTaskSchema) -> VkTask:
...
async def get_list(
    ...
    if uuid_in:
        where.append(VkTask.uuid.in_(uuid_in))

    stmt = select(VkTask).where(and_(*where))
